...
9. **No Inline Fully Qualified Names (FQNs)**: Never use inline fully qualified class/function/property names (e.g. `com.iqbalwork.robithoh.core.database.rememberRobithohDatabase()`). Always `import` at the top of the file and call symbols directly.
...
* **Implementation Plans**: [`plan/`](plan/) (granular step-by-step task lists before execution).
...
* **Self-Correction Diagnostic Parser**: [`tools/extract_build_errors.py`](tools/extract_build_errors.py) (surgical error diagnostic parser for self-fix loop).
* **Hardcoded Strings Linter**: [`tools/check_hardcoded_strings.sh`](tools/check_hardcoded_strings.sh) (auto-ratchet scanner for raw UI strings).
* **Markdown Assets Validator**: [`tools/check_markdown_assets.sh`](tools/check_markdown_assets.sh) (validates 48 pre-bundled liturgical Markdown docs).
* **Verification Pipeline**: [`tools/verify_build.sh`](tools/verify_build.sh) (compilation & allTests runner).
* **Git Hook Installer**: [`tools/install_hooks.sh`](tools/install_hooks.sh) (installs pre-commit verification hook).
* **Maestro E2E Runner**: [`tools/run_maestro.sh`](tools/run_maestro.sh) (executes UI flows on connected device).
* **Memory & State**: [`memory/STATE.md`](memory/STATE.md) (active session context & pending roadmap).
