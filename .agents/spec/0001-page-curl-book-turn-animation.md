...
- **REQ-008 (Dual-Page Foldable & Tablet Mode)**: In dual-page mode (`maxWidth >= DUAL_PAGE_WIDTH_THRESHOLD`), transitions between two-page spreads MUST support 3D paper curl animation: the left page (even) peels toward the central spine revealing the destination spread's left page beneath it, while the right page cross-fades into the destination spread's right page.
