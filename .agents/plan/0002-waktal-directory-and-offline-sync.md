...
- [ ] Modifikasi `shared/src/commonMain/sqldelight/com/iqbalwork/robithoh/core/database/RobithohDatabase.sq`:
...
- [ ] Buat berkas seed baseline `shared/src/commonMain/composeResources/files/waktal_seed.json`:
  - Format JSON array snapshot data awal Wakil Talqin (398 record).
- [ ] Update baseline SHA-256 pada `.agents/rules/sacred_texts_hashes.json` menggunakan:
  ```bash
  bash .agents/tools/check_sacred_texts.sh --update
  ```

### Phase 2: Remote DTOs & Ktor API Client
- [ ] Buat `shared/src/commonMain/kotlin/com/iqbalwork/robithoh/feature/waktal/data/remote/WaktalDto.kt`:
  - `@Serializable BaseApiResponse<T>`
  - `@Serializable PaginationMeta`
  - `@Serializable WakilTalqinItemDto`
  - `@Serializable WakilTalqinDetailDto`
  - `@Serializable SyncManifestResponseDto`
- [ ] Buat `shared/src/commonMain/kotlin/com/iqbalwork/robithoh/feature/waktal/data/remote/WaktalApiService.kt`:
  - Method `checkVersionManifest()`
  - Method `fetchWaktalDelta()`
  - Method `fetchWaktalDetail(id: Int)`

### Phase 3: Domain Logic, Proximity Engine & Repository
- [ ] Buat `shared/src/commonMain/kotlin/com/iqbalwork/robithoh/feature/waktal/domain/HaversineDistance.kt`:
...
- [ ] Buat `shared/src/commonMain/kotlin/com/iqbalwork/robithoh/feature/waktal/domain/WakilTalqin.kt`:
  - Domain class immutable `WakilTalqin` & enum `WaktalStatus`.
  - Mapper function dari DTO / SQLDelight entity ke domain model.
- [ ] Buat `shared/src/commonMain/kotlin/com/iqbalwork/robithoh/feature/waktal/data/WaktalRepository.kt`:
...
- [ ] Buat `shared/src/commonMain/kotlin/com/iqbalwork/robithoh/feature/waktal/data/sync/WaktalSyncManager.kt`:
...
- [ ] Daftarkan dependensi di `shared/src/commonMain/kotlin/com/iqbalwork/robithoh/di/NetworkModule.kt` & `ViewModelModule.kt`.

### Phase 4: MVI Presentation & UI Components
- [ ] Buat kontrak MVI di `feature/waktal/presentation/`:
...
- [ ] Buat komponen UI modular di `feature/waktal/ui/components/`:
...
- [ ] Buat Screen & Content Composable (strict MVI separation):
  - `WaktalListScreen.kt` & `WaktalListContent.kt`.
  - `WaktalDetailScreen.kt` & `WaktalDetailContent.kt`.
- [ ] Daftarkan string resources di `shared/src/commonMain/composeResources/values/strings.xml`.

### Phase 5: Navigation & Home Grid Integration
- [ ] Modifikasi `shared/src/commonMain/kotlin/com/iqbalwork/robithoh/navigation/ScreenKey.kt`:
  - Tambahkan `data object WaktalList : ScreenKey`
  - Tambahkan `data class WaktalDetail(val id: Int) : ScreenKey`
- [ ] Modifikasi `shared/src/commonMain/kotlin/com/iqbalwork/robithoh/navigation/Screens.kt` & `App.kt`:
  - Tambahkan entri rute Navigation3 untuk `ScreenKey.WaktalList` dan `ScreenKey.WaktalDetail`.
- [ ] Modifikasi `shared/src/commonMain/kotlin/com/iqbalwork/robithoh/feature/home/ui/HomeTabContent.kt`:
...
- [ ] Buat unit tests di `shared/src/commonTest/kotlin/`:
  - `com.iqbalwork.robithoh.feature.waktal.HaversineDistanceTest`: Validasi akurasi jarak dan formatting.
  - `com.iqbalwork.robithoh.feature.waktal.WaktalRepositoryTest`: Validasi query filter, search substring, dan seeding JSON.
  - `com.iqbalwork.robithoh.feature.waktal.WaktalViewModelTest`: Validasi transisi state MVI, filter status, dan sorting jarak.
- [ ] Jalankan verifikasi otomatis:
  ...
  ```
- [ ] Sinkronisasi dokumentasi audit di `.agents/changelog/AI_CHANGELOG.md` dan `.agents/memory/STATE.md`.
