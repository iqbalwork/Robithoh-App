...
fun QiblaContent(
    ...
    val deltaAngle = ((deviceHeading.toDouble() - qiblaAzimuth + 540.0) % 360.0) - 180.0
                    ...
                    Column(
                        ...
                        Row(
                            ...
                            Surface(
                                ...
                                color = if (isGpsActive) Color(0xFFDDF5E6) else (if (isDark) DarkSurfaceVariant else Color(0xFFF0F0F0)),
                                modifier = Modifier.clickable { onGpsClick() }
                            ) {
                                Row(
                                    verticalAlignment = Alignment.CenterVertically,
                                    modifier = Modifier.padding(horizontal = 10.dp, vertical = 5.dp)
                                ) {
                                    Text(if (isFetchingLocation) "⏳" else "📍", fontSize = 12.sp)
                                    Spacer(modifier = Modifier.width(4.dp))
                                    Text(
                                        text = if (isFetchingLocation) "Mencari..." else (if (isGpsActive) "GPS Aktif" else "Pakai GPS"),
                                        fontSize = 11.5.sp,
                                        fontWeight = FontWeight.SemiBold,
                                        color = if (isGpsActive) Color(0xFF1E824C) else MerahMerdeka
                                    )
                                }
                            }
                        }
                        ...
                        Row(
                            ...
                            Surface(
                                ...
                                modifier = Modifier.clickable { onCalibrationClick() }
                            ) {
                                Row(
                                    verticalAlignment = Alignment.CenterVertically,
                                    modifier = Modifier.padding(horizontal = 8.dp, vertical = 3.dp)
                                ) {
                                    val (label, textColor) = when (compassAccuracy) {
...
private fun QiblaContentPreview() {
    RabithohTheme {
        QiblaContent(
            qiblaInfo = QiblaInfo(
                directionDegrees = 295.14,
                compassHeading = "Barat Laut (295°)",
                distanceKm = 7921.0,
